...
#start = 86
#end = 87
#for SeriesID in range(start, end):
for SeriesID in seriesList:
    ...
    if not error:
        ...
        ScrapeFunctions.PrintTable(seriesWriter, seriesColumnList, series, " ", ".*' on Amazon.*", "|_|")
        ScrapeFunctions.PrintJunctionTable(seriesPubWriter, SeriesID, [int(series["PublisherID"])])
    time.sleep(0.5)
